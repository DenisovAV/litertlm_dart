...
/// Minimal metrics captured from a conversation's native benchmark info.
///
/// Mirrors flutter_gemma's `SessionMetrics` (core/flutter_gemma_interface.dart)
/// field-for-field so adapters can map 1:1 without loss.
...
///
/// Inlined from flutter_gemma's `SdkTextExtractor` — single source of truth
/// shared with the web `@litert-lm/core` path so both engines map identical
/// chunks to text the same way.
String _extractTextFromResponse(String jsonStr) {
...
class LiteRtLmFfiClient {
  LiteRtLmFfiClient({
    ...
    bool captureNativeLogInDebug = true,
    // Public params (`log`, `androidNativeLibDir`, `captureNativeLogInDebug`)
    // intentionally map to private fields, so no initializing formals.
    // ignore_for_file: prefer_initializing_formals
  }) : _log = log,
       _androidNativeLibDir = androidNativeLibDir,
       _libraries = libraries ?? LibraryLocator.defaultForPlatform(),
       _captureNativeLogInDebug = captureNativeLogInDebug;

  /// Sink for all runtime diagnostics (library loading, engine creation
  /// timing, captured native stderr). Defaults to stdout.
  final LogSink _log;

  /// Android-only: the app's native library directory (where Native Assets
  /// unpacks bundled `.so` files), needed so [Backend.npu] can dlopen
  /// `libLiteRtDispatch_Qualcomm.so` from there. A Flutter adapter obtains
  /// this via a platform channel and injects it; a pure-Dart caller can pass
  /// it directly.
  final String? _androidNativeLibDir;

  /// Resolves the main/proxy native library paths per platform. Defaults to
  /// [LibraryLocator.defaultForPlatform] (app-bundle / Native Assets layout).
  final LibraryLocator _libraries;

  /// Whether to redirect native stderr to a temp file and dump it through
  /// [_log] on engine-create success/failure (debug builds only). Lets
  /// release builds disable the capture entirely.
  final bool _captureNativeLogInDebug;
  ...
  void _dumpNativeLog() {
    ...
    try {
      final f = File(p);
      if (!f.existsSync()) {
        _log('[LiteRtLmFfi/native] log file missing: $p');
        return;
      }
      final content = f.readAsStringSync();
      if (content.isEmpty) {
        _log('[LiteRtLmFfi/native] (no new native log output)');
        return;
      }
      _log(
      ...
      for (var i = 0; i < content.length; i += chunkSize) {
        final end = (i + chunkSize < content.length)
            ? i + chunkSize
            : content.length;
        _log(content.substring(i, end));
      }
      _log('[LiteRtLmFfi/native] === END native log ===');
      ...
      _log('[LiteRtLmFfi/native] failed to read $p: $e');
    }
  }
  ...
  void _ensureBindings() {
    ...
    // output through _log after engine_create failure. Skipped in
    ...
    if (_captureNativeLogInDebug &&
      ...
      if (rc != 0) {
        ...
        _log('[LiteRtLmFfi] stderr redirected to $_nativeLogPath');
      }
    }
    ...
    _log('[LiteRtLmFfi] Libraries loaded');
  }
  ...
  Future<void> initialize({
    ...
    _log('[LiteRtLmFfi/perf] _ensureBindings: ${bindingsMs}ms');
    final b = _bindings!;
    ...
    try {
      ...
      if (Platform.isAndroid && backend == 'npu') {
        final nativeLibDir = _androidNativeLibDir;
        if (nativeLibDir == null) {
          throw StateError(
            '[LiteRtLmFfi] NPU Android: androidNativeLibDir was not provided — '
            'cannot locate libLiteRtDispatch_Qualcomm.so.',
          );
        }
        ...
        calloc.free(dirPtr);
        _log('[LiteRtLmFfi] NPU Android: dispatch_lib_dir=$nativeLibDir');
      }
      ...
      // The engine is created on a background isolate so a slow model load
      // doesn't block the caller. Only sendable values cross the boundary:
      // the settings pointer as an int address, and the main-library PATH
      // (a String) from the locator — NOT `_libraries` or `_log` (an
      // Isolate.run closure runs in a fresh isolate and cannot capture
      // parent state). The isolate re-opens the library by that path so
      // `LibraryLocator.fromDirectory` (CLI) resolves the same file the main
      ...
      _log('[LiteRtLmFfi] Engine initialized successfully');
  ...
  LiteRtLmConversationHandle createConversationHandle({
    ...
    _log('[LiteRtLmFfi] Conversation created');
  ...
  /// Extract text from a LiteRT-LM JSON response chunk. Delegates to the
  /// file-private [_extractTextFromResponse] — single source of truth
  /// shared with the web `@litert-lm/core` path so both engines map
  /// identical chunks to text the same way.
  static String extractTextFromResponse(String jsonStr) =>
      _extractTextFromResponse(jsonStr);
  ...
  void _cancelOn(Pointer<LiteRtLmConversation> conv) {
    if (_bindings != null) {
      ...
      _log('[LiteRtLmFfi] Generation cancelled');
    }
  }
  ...
  void _deleteConversation(Pointer<LiteRtLmConversation> conv) {
    if (_bindings != null) {
      ...
      _log('[LiteRtLmFfi] Conversation closed');
    }
  }
  ...
  void shutdown() {
    ...
    if (_engine != null && _engine != nullptr && _bindings != null) {
      ...
      _engine = null;
      _log('[LiteRtLmFfi] Engine deleted');
    }
  ...
  SessionMetrics _getMetricsOn(Pointer<LiteRtLmConversation> conv) {
    ...
    try {
      ...
      _log('[LiteRtLmFfiClient] Error getting metrics: $e');
      _bindings!.litert_lm_benchmark_info_delete(benchmarkInfo);
      return SessionMetrics();
    }
  }
