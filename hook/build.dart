...
// This hook OWNS the shared libLiteRtLm bundle: it fetches per-platform
// prebuilts from the GitHub release, SHA256-verifies them, and registers the
// CodeAssets. Extracted from flutter_gemma_litertlm's hook (which will drop
// its copy in phase 2 and consume this package instead).
//
// PHASE-2 RECONCILIATION (single-registrant coordination): the shared-cache
// marker records an `owner` package name, and `flutter_gemma_embeddings`
// shares this same libLiteRtLm bundle as a NON-owner (it populates the cache
// but must not re-register the dylib). Today embeddings expects the owner to
// be "flutter_gemma_litertlm". When the flutter_gemma monorepo migrates to
// consume litertlm_dart, either (a) litertlm_dart becomes the owner and
// embeddings' non-owner check is updated, or (b) the owner string is kept
// compatible. The marker FILENAME below ('.flutter_gemma_native_version') is
// the cross-package rendezvous point and is deliberately kept STABLE.
const _packageName = 'litertlm_dart';
